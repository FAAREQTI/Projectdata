...
##For the testing 
def load_csv2(filename: str) -> pd.DataFrame:
    """Loads a CSV file as a pandas DataFrame."""
    try:
        df = pd.read_csv(filename)
        return df
...
def forecasting2(df):
    ...
    for city in cities:
        for product_line in product_lines:
            subset = df[(df['city'] == city) & (df['product_line'] == product_line)]
            subset = subset[['date', 'quantity']].rename(columns={'date': 'ds', 'quantity': 'y'})

            if len(subset) < 3:
                continue

            model = sm.tsa.ARIMA(subset['y'], order=(1, 1, 1))
            model_fit = model.fit()

            future_dates = pd.date_range(subset['ds'].iloc[-1] + pd.Timedelta(days=1), periods=60, freq='D')
            forecast = model_fit.predict(start=len(subset), end=len(subset)+59, typ='levels', dynamic=False)

            forecast_df = pd.DataFrame({'ds': future_dates, 'yhat': forecast})
            forecast_df['city'] = city
            forecast_df['product_line'] = product_line

            forecasts.append(forecast_df[['city', 'product_line', 'ds', 'yhat']])
